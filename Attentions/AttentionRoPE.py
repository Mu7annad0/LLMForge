...
class MultiHeadAttentionRoPE(nn.Module):
    ...
    def __init__(self, d_in, d_out, context_length, num_heads, dtype=None, qkv=False):
        ...
        self.q_proj = nn.Linear(d_in, d_out, bias=qkv, dtype=dtype)
        self.k_proj = nn.Linear(d_in, d_out, bias=qkv, dtype=dtype)
        self.v_proj = nn.Linear(d_in, d_out, bias=qkv, dtype=dtype)
        self.out_proj = nn.Linear(d_out, d_in, bias=qkv, dtype=dtype)
    ...
    def forward(self, x):
        ...
        q = self.q_proj(x).view(b, num_tokens, self.num_heads, self.head_dim) # Queries
        k = self.k_proj(x).view(b, num_tokens, self.num_heads, self.head_dim) # Keys
        v = self.v_proj(x).view(b, num_tokens, self.num_heads, self.head_dim) # Values
